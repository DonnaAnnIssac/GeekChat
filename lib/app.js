...
io.on('connection', client => {
  ...
  client.on('current network', (to, members, room) => {
    let clientid
    for (let clientSockId in clientSocks) {
      if (clientSocks[clientSockId] === client.id) {
        clientid = clientSockId
    ...
    if (!to) {
      client.to(room).emit('current network', members, clientid)
    }
    client.to(clientSocks[to]).emit('current network', members, clientid)
  })
